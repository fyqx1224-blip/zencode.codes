...
async function genReport(){
  ...
  // ── SSE 請求函數（支援自動重試 + 並發鎖等待）──
  async function fetchAnalyzeSSE(body, maxRetries=4){
...
');
            ...
            for (const line of lines) {
              ...
              if (line.startsWith('data: ')) {
                ...
                if (payload === '[DONE]') { resolve(htmlLines.join('
')); return; }
                // 解析 SSE 事件（限流 / 錯誤）
                if (payload.startsWith('{')) {
                  try {
                    ...
                    if (obj.retryAfter !== undefined) {
                      reject(Object.assign(new Error('rate_limit'), { retryAfter: obj.retryAfter }));
                      return;
                    }
                    if (obj.message) { reject(new Error(obj.message)); return; }
                  } catch {}
                }
                htmlLines.push(payload);
                eventType = '';
              }
            }
          }
          if (htmlLines.length > 0) resolve(htmlLines.join('
'));
  ...
  // ── 帶倒計時 UI 的重試包裝 ──
  async function fetchWithRetrySSE(body, maxRetries=4){
    const tl = k => typeof ZCI18n!=='undefined' ? ZCI18n.t(k) : k;
    const phrEl = document.getElementById('lphr');

    for(let attempt=0; attempt<=maxRetries; attempt++){
      try {
        return await fetchAnalyzeSSE(body, 0); // 單次嘗試
      } catch(e) {
        if(e.message === 'rate_limit'){
          if(attempt >= maxRetries) throw e;
          const wait = e.retryAfter || 15;
          let remaining = wait;
  ...
  try{
    const html = await fetchWithRetrySSE(bd);
